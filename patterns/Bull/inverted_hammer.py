...
class InvertedHammer:
    ...
    def check_condition(self, df):
        ...
        - Первые 2 свечи зелёные.
        - Третья свеча красная и меньше четвёртой.
        - Последние 3 свечи красные.
        
        :param df: DataFrame с историческими данными о свечах.
        :return: True, если условие соблюдается, иначе False.
        """
        # Проверяем первые 2 свечи (зелёные)
        first_two_green = all(df.iloc[i]["close"] > df.iloc[i]["open"] for i in range(2))
        
        # Проверяем, что третья свеча красная и меньше четвёртой
        third_red_and_less_than_fourth = (df.iloc[2]["close"] < df.iloc[2]["open"]) and (df.iloc[2]["close"] < df.iloc[3]["close"])
        
        # Проверяем последние 3 свечи (красные)
        last_three_red = all(df.iloc[i]["close"] < df.iloc[i]["open"] for i in range(3, 6))
        
        return first_two_green and third_red_and_less_than_fourth and last_three_red
