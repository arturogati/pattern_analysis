...
class DarkStormCloud:
    ...
    def check_condition(self, df):
        ...
        first_three_red = all(df.iloc[i]["close"] < df.iloc[i]["open"] for i in range(3))
        
        # Проверяем, что третья свеча больше четвёртой
        third_greater_than_fourth = df.iloc[2]["close"] > df.iloc[3]["close"]
        
        # Проверяем последние 3 свечи (зелёные)
        last_three_green = all(df.iloc[i]["close"] > df.iloc[i]["open"] for i in range(3, 6))
        
        return first_three_red and third_greater_than_fourth and last_three_green
...
if __name__ == "__main__":
    # Создаем объект класса DarkStormCloud
    scanner = DarkStormCloud()
